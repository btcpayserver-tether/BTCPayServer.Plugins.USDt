...
{
    private static ChainName ResolveExternalChainNetwork(ChainName chainName)
    {
        // BTCPay regtest has no matching external chain, so use the existing public testnet defaults.
        return chainName == ChainName.Regtest ? ChainName.Testnet : chainName;
    }

    private static TronUSDtLikeConfigurationItem GetTronUSDtHardcodedConfig(ChainName chainName)
    {
        chainName = ResolveExternalChainNetwork(chainName);
        const string logo =
