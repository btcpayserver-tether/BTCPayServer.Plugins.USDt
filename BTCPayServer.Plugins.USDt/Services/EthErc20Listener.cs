    ...
    PaymentService paymentService) : IHostedService
{
                    ...
                    else
                    {
                        var block =
                            await web3Client.Eth.Blocks.GetBlockWithTransactionsByNumber.SendRequestAsync(
                                new BlockParameter(new HexBigInteger(new BigInteger(listenerState.LastBlockHeight + 1))));
    ...
    private async Task SetTrackingState(EthErc20LikeConfigurationItem config, EthErc20ListenerState trackingState)
    {
        await settingsRepository.UpdateSetting(trackingState, EthErc20RPCProvider.ListenerStateSettingKey(config));
    }

    private async Task<EthErc20ListenerState?> LoadTrackingState(EthErc20LikeConfigurationItem config)
    {
        return await settingsRepository.GetSettingAsync<EthErc20ListenerState>(EthErc20RPCProvider.ListenerStateSettingKey(config));
    }
    ...
    private EthErc20LikeConfigurationItem GetConfig(PaymentMethodId pmi)
    {
    ...
    private async Task UpdatePaymentStates(PaymentMethodId pmi, InvoiceEntity[] invoices, BlockWithTransactions block)
    {
        ...
        var handler = (EthErc20PaymentMethodHandler)handlers[pmi];
        ...
        string txId, int confirmations, long blockHeight, InvoiceEntity invoice)
    {
        ...
        var handler = (EthErc20PaymentMethodHandler)handlers[pmi];
        EthErc20PaymentData details = new()
        {
            To = to,
            From = from,
            TransactionId = txId,
